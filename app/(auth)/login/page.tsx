  ...
  return (
    <div className="w-full text-slate-100">
      ...
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/10">
        <div className="text-lg font-semibold border-b-2 border-brand-red pb-4 -mb-4.25 cursor-pointer">Login</div>
        <Link href="/register" className="text-lg font-medium text-slate-400 hover:text-white transition pb-4 -mb-4.25 cursor-pointer">Register</Link>
      ...
      >
          ...
          {(field) => (
            <div>
              ...
              {field.state.meta.errors.length > 0 && (
                <p className="text-red-400 text-sm mt-2 ml-2" role="alert">{String(field.state.meta.errors[0])}</p>        
              )}
            </div>
          )}
        </form.Field>
        ...
        <div className="flex items-center justify-between mt-6">
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" className="rounded bg-white/5 border-white/20 text-brand-red focus:ring-brand-red" />
            <span className="text-sm text-slate-300">Remember me</span>
          </label>
          <a href="#" className="text-sm text-brand-red hover:underline">Forgot password?</a>
        </div>
      ...
      <div className="mt-8 pt-6 border-t border-white/10 text-center">
        ...
        <div className="flex justify-center gap-4">
           {/* Mock Social Buttons */}
           <button className="w-12 h-12 rounded-full bg-white text-slate-900 flex items-center justify-center font-bold text-xl hover:bg-slate-200 transition">G</button>
           <button className="w-12 h-12 rounded-full bg-[#0A66C2] text-white flex items-center justify-center font-bold text-xl hover:bg-blue-700 transition">in</button>
        </div>
      </div>
    </div>
  );

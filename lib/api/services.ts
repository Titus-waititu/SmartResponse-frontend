...
export const accidentService = {
  reportAccident: async (data: AccidentReportRequest): Promise<Accident> => {
    ...
    const response = await apiClient.post(API_CONFIG.ENDPOINTS.ACCIDENTS.REPORT, formData, {
      headers: {
...
export const emergencyServiceService = {
  ...
  createEmergencyService: async (data: Partial<EmergencyService>): Promise<EmergencyService> => {
...
export const notificationService = {
  ...
  markAsRead: async (id: string): Promise<Notification> => {
    const response = await apiClient.patch(API_CONFIG.ENDPOINTS.NOTIFICATIONS.BY_ID(id), {
      isRead: true,
    });
    return response.data;
  },

...
export function UserReportList() {
  ...
  return (
    <div className="space-y-4">
      {reports.map((report) => (
        <div key={report.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-brand-red/50 dark:hover:border-brand-red/50 transition bg-slate-50 dark:bg-slate-800/50 group">
           <div className="flex-1 mb-4 sm:mb-0 pr-4">
             <div className="flex items-center gap-3 mb-2">
                <span className="text-xs font-bold bg-slate-200 dark:bg-slate-700 px-2 py-1 rounded text-slate-600 dark:text-slate-300">#{report.id.substring(0, 8)}</span>
                <ReportStatusBadge status={report.status} />
             </div>
             <p className="text-sm text-slate-700 dark:text-slate-300 font-medium mb-1 line-clamp-1">{report.description || "No description provided."}</p>
           ...
           <div className="sm:text-right">
              ...
              <div className="hidden sm:flex items-center justify-end gap-2 mb-2">
                ...
                <span className={`text-sm font-bold px-2 py-0.5 rounded ${report.severityScore && report.severityScore > 70 ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' : 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400'}`}>
